...
from common_regexes import INT_REGEX
...
DCL_TIMESTAMP_REGEX = r'(\d{4}\/\d{2}\/\d{2}\s\d{2}:\d{2}:\d{2})'
NEWLINE = r'(?:\r\n|\n)?'
INT = r'([+-]?[0-9]+)'

# Regex pattern for a line with the active column list
COLS_PATTERN = (
        r'(\[presf:\w+\]:)' +  # DCL logger ID
...
DATA_PATTERN = (
        DCL_TIMESTAMP_REGEX + r',' +  # PRESF Date and Time
        INT +  # PRESF Unix time (milliseconds since 1/1/1970)
        r'((?:,(?:[+-]?(?:[0-9]*[.])?[0-9]+))+)' +  # PRESF data group (variable size of comma seperated floats)
        NEWLINE
)
...
class PresfDeDclParticleKey(BaseEnum):
    ...
    TEMPERATURE_00 = 'instrument_temperature',
    PRESSURE_00 = 'instrument_pressure',
    TEMPERATURE_01 = 'seawater_temperature',
    SEAPRESSURE_00 = 'pressure',
    DEPTH = 'depth',
    PERIOD_00 = 'pressure_signal_period',
    PERIOD_01 = 'temperature_signal_period'


class DataParticleType(BaseEnum):
    PRESF_DE_PARTICLE_TYPE = 'presf_de_hourly_averaged_instrument'
...
class PresfDeAveragedDclParser(SimpleParser):
    """
    Presf_de DCL Parser.
    """
    ...
    def __init__(self, config, stream_handle, exception_callback):
        ...
        super(PresfDeAveragedDclParser, self).__init__(config, stream_handle, exception_callback)
    ...
    def parse_record(self, record):
        ...
        try:
            ...
            instrument_time = str(record.group(1) + '.000')

            # Remaining data correspond to active channels (default or previously read from log)
            channel_data_list = record.group(3).strip('\n').split(',')[1:]

            # Make sure that the data list and the channel list are the same size, otherwise data might be incomplete
            if len(channel_data_list) == len(self.active_channels):
